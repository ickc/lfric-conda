...
# Target tree: vendor/lfric_apps (staged by scripts/stage-sources.sh).
#
...
# anything: external sources are pinned checkouts the user stages explicitly
# (`bash scripts/stage-sources.sh`, from the refs in sources.yaml). get_source
...
# The minimal-compile example patches the staged vendor/ trees in place; the
...
def get_source(
    ...
    External sources are pinned checkouts the user stages explicitly
    (``bash scripts/stage-sources.sh``, from the refs in sources.yaml). This
    ...
    if ".git" in source:
        raise RuntimeError(
            ...
            f"stage '{repo}' as a pinned checkout (got source={source!r}). "
            f"Run: bash scripts/stage-sources.sh"
        )
